  ...
  try {
    ...
    try { data = JSON.parse(rawText); }
      ...
      return res.send(`<html><body style="font-family:monospace;padding:20px">
        ...
        <pre style="background:#f0f0f0;padding:10px;word-break:break-all;white-space:pre-wrap">${rawText.slice(0,2000)}</pre>
      </body></html>`);
    }
    ...
    try {
      ...
      return (d.data?.products || []).slice(0, 5).map(p => ({
        ...
        price:         p.product_price
          ? parseFloat(p.product_price.replace(/[^0-9.]/g, '')) * markup
          : null,
        originalPrice: p.product_price
          ? parseFloat(p.product_price.replace(/[^0-9.]/g, ''))
          : null,
