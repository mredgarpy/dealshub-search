/* StyleHub Most Wanted Page v1.0 — Sprint 5
   Uses Amazon MOST_WISHED_FOR best-sellers type with category tabs + heart icon + wish count
*/
(function(){
  'use strict';
  ...
  function productCard(p, idx){
    ...
    // Simulated wish count based on rank
    var wishCount = Math.max(50, Math.floor(500 / Math.max(rank, 1)) + Math.floor(Math.random() * 80));
    ...
    // Heart + wish count badge
    h += '<span style="position:absolute;top:8px;left:8px;background:rgba(236,72,153,.9);color:#fff;padding:3px 10px;border-radius:4px;font-size:10px;font-weight:700;display:flex;align-items:center;gap:4px">\u2764 ' + wishCount + ' want this</span>';
    // Origin badge
    ...
    if(rank <= 10) h += '<span style="position:absolute;bottom:8px;right:8px;background:#ec4899;color:#fff;padding:2px 8px;border-radius:50%;font-size:10px;font-weight:700;min-width:20px;text-align:center">#' + rank + '</span>';
    h += '</div>';
  ...
  function renderTabs(){
    ...
    categories.forEach(function(c){
      ...
      var bg = isActive ? '#ec4899' : '#fff';
      var color = isActive ? '#fff' : '#374151';
      var border = isActive ? '#ec4899' : '#e2e8f0';
  ...
  function loadCategory(catId){
    ...
    fetch(API + '/api/amazon-bestsellers?type=MOST_WISHED_FOR&category=' + encodeURIComponent(catId) + '&limit=30', {signal: AbortSignal.timeout(15000)})
      ...
      .catch(function(){
        ...
        contentEl.innerHTML = '<div style="text-align:center;padding:40px;color:#6b7280"><p style="font-size:16px">Unable to load most wanted items</p><p style="font-size:13px">Please try again later</p></div>';
      });
  }
  ...
  function renderProducts(items){
    ...
    if(!contentEl) return;
    if(!items.length){
      contentEl.innerHTML = '<div style="text-align:center;padding:40px;color:#6b7280"><p>No most wanted items found in this category</p></div>';
      return;
    }
    var h = '<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:16px" class="dh-mw-grid">';
    items.forEach(function(p, idx){ h += productCard(p, idx); });
    h += '</div>';
  ...
  style.textContent = '@media(max-width:768px){.dh-mw-grid{grid-template-columns:repeat(2,1fr)!important;gap:10px!important}}';
