...
class AliExpressAdapter extends BaseAdapter {
  ...
  normalizeProduct(d) {
    try {
      ...
      p.breadcrumbs = Array.isArray(d.breadcrumbs) ? d.breadcrumbs.map(b => b.name || b) : [];
      p.description = d.description || '';
      p.bullets = Array.isArray(d.features) ? d.features : [];

      const imgs = d.images || d.imagePathList || [];
      p.images = (Array.isArray(imgs) ? imgs : []).map(img => typeof img === 'string' ? img : img?.imgUrl || '').filter(Boolean);
      ...
      logger.error('aliexpress', 'normalizeProduct error', { error: e.message });
      return null;
    }
  }
