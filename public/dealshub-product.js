...
function renderProduct(p){
  ...
  if(p.options&&p.options.length){
    ...
    for(var oi=0;oi<p.options.length;oi++){
      ...
      html+='<div style="margin-bottom:12px"><label style="font-size:14px;font-weight:600;color:#333;display:block;margin-bottom:6px">'+escHTML(opt.name)+'</label>';
      html+='<div style="display:flex;flex-wrap:wrap;gap:8px">';
      for(var vi=0;vi<(opt.values||[]).length;vi++){
        var val=opt.values[vi];
        var sel=val.selected?' dhpdp-opt-sel':'';
        if(val.image){
          html+='<button class="dhpdp-opt'+sel+'" data-option="'+oi+'" data-value="'+vi+'" style="width:40px;height:40px;border-radius:8px;border:2px solid '+(val.selected?'#e53e3e':'#ddd')+';padding:2px;cursor:pointer;background:#fff"><img src="'+escHTML(val.image)+'" style="width:100%;height:100%;object-fit:cover;border-radius:6px"></button>';
        }else{
          html+='<button class="dhpdp-opt'+sel+'" data-option="'+oi+'" data-value="'+vi+'" style="padding:8px 16px;border-radius:8px;border:2px solid '+(val.selected?'#e53e3e':'#ddd')+';cursor:pointer;background:'+(val.selected?'#fef2f2':'#fff')+';font-size:13px;color:#333">'+escHTML(val.value)+'</button>';
        }
      }
      html+='</div></div>';
    }
  ...
  if(desc||bullets.length){
    ...
    if(bullets.length){
      html+='<div id="dhpdp-tab-features" class="dhpdp-tabcontent" style="'+(desc?'display:none':'')+'">';
...
function bindEvents(p,imgs){
  ...
  function doAddToCart(buyNow,isRetry){
    ...
    .then(function(result){
      ...
      btn.textContent='\u2713 Added!';btn.style.background='#22c55e';
      setTimeout(function(){
