/* StyleHub Most Wanted Page v2.0 — Infinite Scroll
   Uses Amazon MOST_WISHED_FOR with category tabs + heart/wish count
...
(function(){
  ...
  var PAGE_SIZE = 20;
  var categories = [
    {id: 'electronics', name: 'Electronics', icon: '🎧', searchTerms: 'popular electronics trending'},
    {id: 'fashion', name: 'Fashion', icon: '👗', searchTerms: 'popular fashion trending clothing'},
    {id: 'beauty', name: 'Beauty', icon: '💄', searchTerms: 'popular beauty skincare trending'},
    {id: 'garden', name: 'Home', icon: '🏠', searchTerms: 'popular home garden decor'},
    {id: 'sporting', name: 'Sports', icon: '⚽', searchTerms: 'popular sports fitness gear'},
    {id: 'videogames', name: 'Gaming', icon: '🎮', searchTerms: 'popular gaming accessories'},
    {id: 'baby-products', name: 'Baby', icon: '👶', searchTerms: 'popular baby kids toys'}
  ];
  var activeCategory = 'electronics';
  var cache = {};

  var allProducts=[];var displayedCount=0;var seenKeys={};var apiPage=0;
  var isFetchingPage=false;var noMoreResults=false;var isLoadingMore=false;var scrollObserver=null;
  ...
  function productCard(p, idx){
    ...
    if(wishCount>0) h+='<span style="position:absolute;top:8px;left:8px;background:rgba(236,72,153,.9);color:#fff;padding:3px 10px;border-radius:4px;font-size:10px;font-weight:700;display:flex;align-items:center;gap:4px">❤ '+wishCount+' want this</span>';
    ...
    if(rank>0&&rank<=10) h+='<span style="position:absolute;bottom:8px;right:8px;background:#ec4899;color:#fff;padding:2px 8px;border-radius:50%;font-size:10px;font-weight:700;min-width:20px;text-align:center">#'+rank+'</span>';
    h+='</div>';
  ...
  function renderTabs(){
    var tabsEl=document.getElementById('dh-mw-tabs');
    if(!tabsEl)return;
    var h='';
    categories.forEach(function(c){
      var isActive=c.id===activeCategory;
      var bg=isActive?'#ec4899':'#fff';var color=isActive?'#fff':'#374151';var border=isActive?'#ec4899':'#e2e8f0';
  ...
  function resetScrollState(){allProducts=[];displayedCount=0;seenKeys={};apiPage=0;isFetchingPage=false;noMoreResults=false;isLoadingMore=false;}
  function addUniqueProducts(items){var count=0;items.forEach(function(p){var key=(p.id||p.sourceId||'')+'_'+(p.source||'');if(!seenKeys[key]){seenKeys[key]=true;allProducts.push(p);count++;}});return count;}

  function resetAndLoad(catId){
    resetScrollState();
    var spinner=document.getElementById('dh-mw-spinner');if(spinner)spinner.style.display='none';
    loadCategory(catId);
  }

  function loadCategory(catId){
    var contentEl=document.getElementById('dh-mw-content');
    var skelEl=document.getElementById('dh-mw-skeleton');
    if(!contentEl)return;
    contentEl.innerHTML='';
    if(skelEl) skelEl.style.display='';

    var doRender=function(items){addUniqueProducts(items);if(skelEl)skelEl.style.display='none';renderProducts();setupInfiniteScroll();};
    if(cache[catId]){doRender(cache[catId]);return;}

    fetch(API+'/api/amazon-bestsellers?type=MOST_WISHED_FOR&category='+encodeURIComponent(catId)+'&limit=30',{signal:AbortSignal.timeout(15000)})
      .then(function(r){if(!r.ok)throw new Error('HTTP '+r.status);return r.json()})
      .then(function(data){var items=data.results||[];cache[catId]=items;doRender(items);})
      .catch(function(){if(skelEl)skelEl.style.display='none';setupInfiniteScroll();fetchNextPage();});
  }

  function renderProducts(){
    var contentEl=document.getElementById('dh-mw-content');
    if(!contentEl)return;
    if(!allProducts.length){contentEl.innerHTML='<div style="text-align:center;padding:40px;color:#6b7280"><p>No most wanted items found</p></div>';return;}
    var nextBatch=allProducts.slice(displayedCount,displayedCount+PAGE_SIZE);
    if(!nextBatch.length){isLoadingMore=false;return;}
    var container=contentEl.querySelector('.dh-mw-grid');
    if(!container){
      var end=Math.min(PAGE_SIZE,allProducts.length);
      var h='<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:16px" class="dh-mw-grid">';
  ...
  function fetchNextPage(){
    ...
    isFetchingPage=true;showSpinner();apiPage++;
    ...
    return Promise.all([
      ...
      var added=addUniqueProducts(newItems);hideSpinner();
      if(added===0){noMoreResults=true;isFetchingPage=false;return false;}
      noMoreResults=!anyHasMore;renderProducts();isFetchingPage=false;return true;
    }).catch(function(){hideSpinner();isFetchingPage=false;return false;});
  }

  function getSentinel(){var s=document.getElementById('dh-mw-sentinel');if(!s){s=document.createElement('div');s.id='dh-mw-sentinel';s.style.cssText='height:1px;width:100%';var c=document.getElementById('dh-mw-content');if(c)c.parentNode.insertBefore(s,c.nextSibling);}return s;}
  function hideSentinel(){var s=document.getElementById('dh-mw-sentinel');if(s)s.style.display='none';}
  function updateSentinel(){var s=getSentinel();if(s)s.style.display='';}
  function showSpinner(){
    var el=document.getElementById('dh-mw-spinner');
    if(!el){el=document.createElement('div');el.id='dh-mw-spinner';var c=document.getElementById('dh-mw-content');if(c)c.parentNode.insertBefore(el,c.nextSibling);}
    el.style.display='';el.innerHTML='<div style="display:flex;align-items:center;justify-content:center;gap:8px;padding:20px"><div style="width:20px;height:20px;border:3px solid #e2e8f0;border-top-color:#ec4899;border-radius:50%;animation:dhspin 0.8s linear infinite"></div><span style="color:#6b7280;font-size:14px">Loading more products...</span></div>';
  }
  function hideSpinner(){var el=document.getElementById('dh-mw-spinner');if(el)el.style.display='none';}
  function showEndMsg(){
    var el=document.getElementById('dh-mw-spinner');
    if(!el){el=document.createElement('div');el.id='dh-mw-spinner';var c=document.getElementById('dh-mw-content');if(c)c.parentNode.insertBefore(el,c.nextSibling);}
    if(displayedCount>PAGE_SIZE){el.style.display='';el.innerHTML='<div style="text-align:center;padding:20px;color:#9ca3af;font-size:13px">You\'ve explored all most wanted products</div>';}
  }
  ...
  style.textContent='@media(max-width:768px){.dh-mw-grid{grid-template-columns:repeat(2,1fr)!important;gap:10px!important}}@keyframes dhspin{to{transform:rotate(360deg)}}';
