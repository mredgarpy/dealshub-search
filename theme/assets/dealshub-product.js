...
(function(){
  ...
  function renderProduct(p){
    ...
    if(discount>0)html+='<span style="position:absolute;top:12px;right:12px;background:#E53E3E;color:#fff;padding:4px 10px;border-radius:20px;font-size:12px;font-weight:600;z-index:2">-'+discount+'%</span>';
  ...
  function addToRecentlyViewed(p){
    try{
      ...
      // Render recently viewed section after a short delay
  ...
  function renderRecentlyViewed(currentId){
    try{
      ...
      var rvHtml='<div style="max-width:1200px;margin:40px auto 0;padding:0 20px">';
      rvHtml+='<h2 style="font-size:20px;font-weight:700;color:#1a1a2e;margin-bottom:16px">Recently Viewed</h2>';
      rvHtml+='<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:14px">';
      items.forEach(function(item){
        ...
        rvHtml+='<a href="'+link+'" style="text-decoration:none;background:#fff;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden;transition:box-shadow .2s">';
        rvHtml+='<div style="aspect-ratio:1;background:#f8f9fa;overflow:hidden"><img src="'+escHTML(item.image||'')+'" alt="" style="width:100%;height:100%;object-fit:contain" onerror="this.parentElement.style.background=\'#f0f0f0\'"></div>';
        rvHtml+='<div style="padding:10px"><div style="font-size:12px;color:#4a5568;line-height:1.3;height:32px;overflow:hidden">'+escHTML((item.title||'').substring(0,60))+'</div>';
        if(prc>0)rvHtml+='<div style="font-size:15px;font-weight:700;color:#e53e3e;margin-top:6px">$'+prc.toFixed(2)+'</div>';
        rvHtml+='</div></a>';
      });
      rvHtml+='</div></div>';
      container.insertAdjacentHTML('beforeend',rvHtml);
    }catch(e){}
  }
})();
