/* StyleHub API v3.1 - Internal Links + sessionStorage cache */
(function(){
'use strict';
var API='https://dealshub-search.onrender.com/api',CACHE_TTL=3e5,cache={},SS_PREFIX='sh_';
function sGet(k){try{var raw=sessionStorage.getItem(SS_PREFIX+k);if(!raw)return null;var e=JSON.parse(raw);if(Date.now()-e.ts>CACHE_TTL){sessionStorage.removeItem(SS_PREFIX+k);return null}return e.data}catch(x){return null}}
function sSet(k,d){try{sessionStorage.setItem(SS_PREFIX+k,JSON.stringify({data:d,ts:Date.now()}))}catch(x){try{var keys=[];for(var i=0;i<sessionStorage.length;i++){var sk=sessionStorage.key(i);if(sk&&sk.indexOf(SS_PREFIX)===0)keys.push(sk)}keys.slice(0,Math.ceil(keys.length/2)).forEach(function(sk){sessionStorage.removeItem(sk)});sessionStorage.setItem(SS_PREFIX+k,JSON.stringify({data:d,ts:Date.now()}))}catch(x2){}}}
function getCached(k){var e=cache[k];if(e&&Date.now()-e.ts<CACHE_TTL)return e.data;var sd=sGet(k);if(sd){cache[k]={data:sd,ts:Date.now()};return sd}return null}
function setCache(k,d){cache[k]={data:d,ts:Date.now()};sSet(k,d)}
...
function card(p,o){p.price=parseFloat(String(p.price).replace(/[^0-9.]/g,''))||0;p.originalPrice=p.originalPrice?parseFloat(String(p.originalPrice).replace(/[^0-9.]/g,'')):0;
  ...
  var _b=typeof p.badge==='string'?p.badge:(p.badge&&p.badge.text?p.badge.text:'');var bdg=_b?'<span style="position:absolute;top:8px;left:8px;background:#e53e3e;color:#fff;font-size:11px;padding:2px 8px;border-radius:4px;font-weight:600;z-index:2">'+dec(_b)+'</span>':'';
  ...
  var href='/pages/product?id='+p.id+'&store='+p.source+(p.title?'&title='+encodeURIComponent(p.title):'');
